...
> 所有指标可由评审用 `backend/tests/` 与 `copilot_e2e.cjs` 复现。
...
| 离线可复现 | Scenario 模式无外部网络可完成三场景 | 是 | 是（copilot_e2e.cjs 三场景全过） |
| 手机端处置可用 | 390×844 视口可完成场景选择、人工确认和审计包导出，无横向溢出 | 是 | 是（copilot_e2e.cjs 自动断言） |
...
见 `run-guide.md` 第 7 节。测试清单：`test_copilot.py`（守门）、`test_copilot_engine.py`（双模式）、`test_copilot_api.py`（运行持久化）、`test_copilot_integration.py`（真实数据库证据链）、`copilot_e2e.cjs`（浏览器全流程）。
